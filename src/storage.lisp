...
(defun todo-to-hash-table (todo)
  ...
  ;; Absent values serialize as JSON null via jzon's 'null symbol; a bare
  ;; NIL would render as false (cloodoo-6jt)
  (flet ((or-null (value) (or value 'null)))
    (let ((ht (make-hash-table :test #'equal)))
      (setf (gethash "id" ht) (todo-id todo))
      (setf (gethash "title" ht) (todo-title todo))
      (setf (gethash "description" ht) (or-null (todo-description todo)))
      ...
      (setf (gethash "scheduled_date" ht) (or-null
                                           (when (todo-scheduled-date todo)
                                             (lt:format-rfc3339-timestring nil (todo-scheduled-date todo)))))
      (setf (gethash "due_date" ht) (or-null
                                     (when (todo-due-date todo)
                                       (lt:format-rfc3339-timestring nil (todo-due-date todo)))))
      (setf (gethash "tags" ht) (or (todo-tags todo) #()))
      (setf (gethash "location_info" ht) (or-null
                                          (location-info-to-hash-table (todo-location-info todo))))
      (setf (gethash "url" ht) (or-null (todo-url todo)))
      (setf (gethash "attachment_hashes" ht) (if (todo-attachment-hashes todo)
      ...
      (setf (gethash "completed_at" ht) (or-null
                                         (when (todo-completed-at todo)
                                           (lt:format-rfc3339-timestring nil (todo-completed-at todo)))))
      (setf (gethash "device_id" ht) (or-null (todo-device-id todo)))
      (setf (gethash "repeat_interval" ht) (or-null (todo-repeat-interval todo)))
      (setf (gethash "repeat_unit" ht) (or-null
